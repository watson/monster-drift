...
function sin (freq, gain, samples) {
  return gen(Math.sin, freq, gain, samples)
}

function cos (freq, gain, samples) {
  return gen(Math.cos, freq, gain, samples)
}

function gen (waveFn, freq, gain, samples) {
  var arr = []
  for (var t = 0; t < samples; t++) {
    arr.push(gain * waveFn(2 * Math.PI * freq * t))
  }
  return arr
}
